...
//  indexed using GLFW_KEY_* codes, e.g. ImGui::IsKeyPressed(GLFW_KEY_SPACE). [X] Platform: Multi-viewport support
//  (multiple windows). Enable with 'io.ConfigFlags |= ImGuiConfigFlags_ViewportsEnable'.

// Issues:
//  [ ] Platform: Multi-viewport support: ParentViewportID not honored, and so io.ConfigViewportsNoDefaultParent has no
//  effect (minor).
...
//  g_XXXX access changed to bd->XXXX). 2020-01-17: Inputs: Disable error callback while assigning mouse cursors because
//  some X11 setup don't have them and it generates errors. 2019-12-05: Inputs: Added support for new mouse cursors
//  added in GLFW 3.4+ (resizing cursors, not allowed cursor). 2019-10-18: Misc: Previously installed user callbacks are
//  now restored on shutdown. 2019-07-21: Inputs: Added mapping for ImGuiKey_KeyPadEnter. 2019-05-11: Inputs: Don't
//  filter value from character callback before calling AddInputCharacter(). 2019-03-12: Misc: Preserve
//  DisplayFramebufferScale when main window is minimized. 2018-11-30: Misc: Setting up io.BackendPlatformName so it can
//  be displayed in the About Window. 2018-11-07: Inputs: When installing our GLFW callbacks, we save user's previously
//  installed ones - if any - and chain call them. 2018-08-01: Inputs: Workaround for Emscripten which doesn't seem to
//  handle focus related calls. 2018-06-29: Inputs: Added support for the ImGuiMouseCursor_Hand cursor. 2018-06-08:
//  Misc: Extracted imgui_impl_glfw.cpp/.h away from the old combined GLFW+OpenGL/Vulkan examples. 2018-03-20: Misc:
//  Setup io.BackendFlags ImGuiBackendFlags_HasMouseCursors flag + honor ImGuiConfigFlags_NoMouseCursorChange flag.
...
//  CreateContext/DestroyContext themselves. 2018-02-06: Inputs: Added mapping for ImGuiKey_Space. 2018-01-25: Inputs:
//  Added gamepad support if ImGuiConfigFlags_NavEnableGamepad is set. 2018-01-25: Inputs: Honoring the
//  io.WantSetMousePos by repositioning the mouse (when using navigation and ImGuiConfigFlags_NavMoveMouse is set).
...
#define GLFW_HAS_MONITOR_WORK_AREA                                                                                     \
    (GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 >= 3300) // 3.3+ glfwGetMonitorWorkarea
#define GLFW_HAS_OSX_WINDOW_POS_FIX                                                                                    \
    (GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 + GLFW_VERSION_REVISION * 10 >=                              \
     3310)                     // 3.3.1+ Fixed: Resizing window repositions it on MacOS #1553
...
#define GLFW_HAS_MOUSE_PASSTHROUGH                                                                                     \
    (GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 >= 3400) // 3.4+ GLFW_MOUSE_PASSTHROUGH
#else
...
static ImGui_ImplGlfw_Data *ImGui_ImplGlfw_CreateBackendData() {
    IM_ASSERT(g_Data == NULL);
    g_Data = IM_NEW(ImGui_ImplGlfw_Data);
    return g_Data;
}
static ImGui_ImplGlfw_Data *ImGui_ImplGlfw_GetBackendData() {
    return ImGui::GetCurrentContext() != NULL ? g_Data : NULL;
}
static void ImGui_ImplGlfw_DestroyBackendData() {
    IM_DELETE(g_Data);
    g_Data = NULL;
}
...
void ImGui_ImplGlfw_CharCallback(GLFWwindow *window, unsigned int c) {
    ImGui_ImplGlfw_Data *bd = ImGui_ImplGlfw_GetBackendData();
    if (bd->PrevUserCallbackChar != NULL && window == bd->Window) bd->PrevUserCallbackChar(window, c);
...
#endif
        if (glfwGetWindowAttrib(window, GLFW_HOVERED) && !window_no_input) io.MouseHoveredViewport = viewport->ID;
#endif
    }
}
...
static LRESULT CALLBACK WndProcNoInputs(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    if (msg == WM_NCHITTEST) {
        // Let mouse pass-through the window. This will allow the backend to set io.MouseHoveredViewport properly (which
        // is OPTIONAL). The ImGuiViewportFlags_NoInputs flag is set while dragging a viewport, as want to detect the
        // window behind the one we are dragging. If you cannot easily access those viewport flags from your
